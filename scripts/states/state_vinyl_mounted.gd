...
func enter_state():
	...
	if gramophone:
		...
		var song_info = "%s - %s" % [song.artist, song.title] if song else "Unknown"
		gramophone.set_instructions("[color=green]¡Vinilo colocado![/color]\n%s\n[color=red]Toca para cambiar[/color]" % song_info)
